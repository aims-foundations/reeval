...
def infer_column_types(df):
    for col in df.columns:
        ...
        if set(unique_values).issubset({"True", "False", "0", "1"}):
            df[col] = (
                df[col]
                .map(lambda x: True if x in ["True", "1"] else False)
                .astype("bool")
            )
...
if __name__ == "__main__":
    ...
    for benchmark in BENCHMARKS:
        ...
        all_paths += [
            f"{input_dir}/{benchmark}/runs/{s}/{r}" for r, s in folder_dict.items()
        ]
    ...
    results = []
    for d_requests, d_predictions, run_specs, instances, paths in tqdm(
        zip(*all_lists, all_paths), total=len(all_lists[0])
    ):
        ...
        run_specs = run_specs.loc[
            run_specs.index.repeat(d_predictions.shape[0])
    ...
    for col in results.columns:
        if results[col].dtype != "category" and results[col].isna().all():
            ...
            if (
                results[col].dtype == "float64"
                and np.nanmax(results[col]) < 65500
                and np.nanmin(results[col]) > -65500
            ):
