...
# df.true.theta <- data.frame(pid = simulated_pid, thetaTrue = theta)
...
```{r}
# df.monte.carlo.simulation.compare <- merge(df.monte.carlo.results.v2, df.true.theta, by = "pid", all.x = TRUE)
# df.monte.carlo.simulation.compare
```

```{r}
# df.compare.all <- df.monte.carlo.simulation.compare %>%
#   select(pid, variant, trialNumTotal, thetaEstimate, iteration, thetaSE, thetaTrue) %>%
#   mutate(variant = ifelse(variant == "adaptive", "CAT", "Random"))
# df.compare.all
```
