...
from utils import item_response_fn_1PL, set_seed, goodness_of_fit_1PL_plot
...
def em_calibration(
    ...
    for _ in pbar:
        ...
        for i in range(num_node):
            ...
            theta_prior = torch.exp(-theta_nodes[i]**2 / 2) / torch.sqrt(torch.tensor(2 * torch.pi))
            prob_matrixes[:,:,i] = prob_matrix * theta_prior * weights[i]
