...
import wandb
from utils import set_seed, split_indices, MLP
...
if __name__ == "__main__":
    ...
    if args.dataset != 'aggregate':
        ...
        main(
            ...
            save_model_path=f'{output_dir}/{args.model}.pkl' if args.seed==0 else None,
        )
        ...
        if args.task == 'byrandom':
            ...
            datasets = [load_dataset('stair-lab/reeval_aggregate-embed', split=split) for split in splits]
            ...
            main(
                ...
                save_model_path=f'{output_dir}/{args.model}_byrandom.pkl' if args.seed==0 else None,
            )
            ...
            train_datasets = [load_dataset('stair-lab/reeval_aggregate-embed', split=split) for split in train_splits]
            train_dataset = concatenate_datasets(train_datasets)
            emb_train = np.array(train_dataset['embed'])
            z_train = np.array(train_dataset['z'])
            
            test_datasets = [load_dataset('stair-lab/reeval_aggregate-embed', split=split) for split in test_splits]
            ...
            main(
                ...
                save_model_path=f'{output_dir}/{args.model}_bydataset.pkl' if args.seed==0 else None,
            )