...
if __name__ == "__main__":
    ...
    base_idx = 0
    for i, start_string in enumerate(start_strings):
        input_file = f"{json_dir}/{start_string},model=AlephAlpha_luminous-base.json"
        with open(input_file, 'r') as f:
            data = json.load(f)
        if len(data['request_states']) != max_lens[i]:
            print("Warning: this model has less questions than max question, please change model.")
        for j, question in enumerate(data['request_states']):
            text = question['instance']['input']['text']
            output_data.append([base_idx+j, text, bool_delete_list[base_idx+j]])
        base_idx += max_lens[i]
