from util import get_everything_benchmark_raw
import torch
import numpy as np
...
def get_metrics_from_raw(config_name,col_mask):
    raw_data_package = load_raw(config_name)
    r_test, test_auc, log_p_test = compute_metrics(raw_data_package['P_hat'][:,col_mask], raw_data_package['Y'][:,col_mask], raw_data_package['test_idtor'][:,col_mask])
    r_train, train_auc, log_p_train = compute_metrics(raw_data_package['P_hat'][:,col_mask], raw_data_package['Y'][:,col_mask], raw_data_package['train_idtor'][:,col_mask])
    return {
...
for dataset in ['everything2']:
    for masking_method in masking_methods:
        
        for K_fit in factors:
            print(f"loading {dataset} {masking_method} {K_fit}")
            ...
            for benchmark in benchmarks:
                ...
                try:
                    ...
                    performance = get_metrics_from_raw(config_name, train_col_idtor)
                except:
...
df.to_csv("results/summary/partial_results_auc_corr_sub_dataset.csv", index=False)
